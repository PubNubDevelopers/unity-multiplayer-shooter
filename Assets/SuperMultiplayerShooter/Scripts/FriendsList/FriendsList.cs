    ...
    {    
        ...
        {                     
           ...
           if(fetchHistoryResponse != null && fetchHistoryResponse.Result != null && fetchHistoryResponse.Result.Messages != null && !fetchHistoryResponse.Status.Error)
            {
                // Iterate over each channel in the Messages dictionary
                foreach (KeyValuePair<string, List<PNHistoryItemResult>> channel in fetchHistoryResponse.Result.Messages)
                {
                    Debug.Log($"Processing entries for channel: {channel.Key}");

                    // Iterate over each PNHistoryItemResult in the channel's list
                    foreach (PNHistoryItemResult item in channel.Value)
                    {
                        // Insert your code to evaluate item.Entry here
                        // Since Entry is of type object, you'll likely need to cast it to its actual type before doing anything with it
